...
void Setup_ADMIN()
{
  while(simSerial.available())
  {
    ...
    if(splitString(inputString,"#",1)=="ADMIN")
    {
      ...
      lcd.clear(); lcd.setCursor(0,0); lcd.print("ADMIN Completed");
      resetFunc(); // reset
    }
  }
}
...
void Setup_MAP()
{
  while(simSerial.available())
  {
    ...
    {
      Val = splitString(inputString,"#",2);
      Serial.println(Val);
      for (int i = 0; i < Val.length(); ++i){EEPROM.write(15+i, Val[i]);} 
      delay(500);

      simSerial.println("AT+CMGF=1"); delay(500);
      simSerial.println("AT+CMGS=\"" + Val + "\"\r"); delay(500);
      simSerial.println("MAP OK! ");  
      simSerial.println((char)26); delay(500);
      lcd.clear(); lcd.setCursor(0,0); lcd.print("MAP Completed");
      resetFunc(); // reset
    }
  }
}
...
{ 
  ...
  simSerial.print("http://maps.google.com/maps?z=18&q="); simSerial.println(MAPS); 
